 ...
 *  C := C + A * B
 * where C is BLOCK_SIZE-by-BLOCK_SIZE, A is M-by-K, and B is K-by-N. 
 * Assume that block dimensions are divisible by 4 */
static void do_block (double* A, double* B, double* C)
{
  __m128d c1,c2,c3,c4,c5,c6,c7,c8,a1,a2,b; 
  /* For each row i of A */
  for (int j = 0; j < BLOCK_SIZE; j+=4)
  {
    ...
    {
      /* Compute C(i,j) */
      c1 = _mm_load_pd(C+i+j*BLOCK_SIZE);
      c2 = _mm_load_pd(C+i+2+j*BLOCK_SIZE);
      c3 = _mm_load_pd(C+i+(j+1)*BLOCK_SIZE);
      c4 = _mm_load_pd(C+i+2+(j+1)*BLOCK_SIZE);
      c5 = _mm_load_pd(C+i+(j+2)*BLOCK_SIZE);
      c6 = _mm_load_pd(C+i+2+(j+2)*BLOCK_SIZE);
      c7 = _mm_load_pd(C+i+(j+3)*BLOCK_SIZE);
      c8 = _mm_load_pd(C+i+2+(j+3)*BLOCK_SIZE);
     
      for (int k = 0; k < BLOCK_SIZE; ++k)
      {
        a1 = _mm_load_pd(A+i+k*BLOCK_SIZE);
        a2 = _mm_load_pd(A+i+2+k*BLOCK_SIZE);
      
        b = _mm_load1_pd(B+j+k*BLOCK_SIZE);
	c1 = _mm_add_pd(_mm_mul_pd(a1,b),c1);
	c2 = _mm_add_pd(_mm_mul_pd(a2,b),c2);

        b = _mm_load1_pd(B+j+1+k*BLOCK_SIZE);
 	c3 = _mm_add_pd(_mm_mul_pd(a1,b),c3);
	c4 = _mm_add_pd(_mm_mul_pd(a2,b),c4);

        b = _mm_load1_pd(B+j+2+k*BLOCK_SIZE);
 	c5 = _mm_add_pd(_mm_mul_pd(a1,b),c5);
	c6 = _mm_add_pd(_mm_mul_pd(a2,b),c6);

        b = _mm_load1_pd(B+j+3+k*BLOCK_SIZE);
 	c7 = _mm_add_pd(_mm_mul_pd(a1,b),c7);
	c8 = _mm_add_pd(_mm_mul_pd(a2,b),c8);
     }
      _mm_store_pd(C+i+j*BLOCK_SIZE,c1);
      _mm_store_pd(C+i+2+j*BLOCK_SIZE,c2);
      _mm_store_pd(C+i+(j+1)*BLOCK_SIZE,c3);
      _mm_store_pd(C+i+2+(j+1)*BLOCK_SIZE,c4);
      _mm_store_pd(C+i+(j+2)*BLOCK_SIZE,c5);
      _mm_store_pd(C+i+2+(j+2)*BLOCK_SIZE,c6);
      _mm_store_pd(C+i+(j+3)*BLOCK_SIZE,c7);
      _mm_store_pd(C+i+2+(j+3)*BLOCK_SIZE,c8);
    }
  }
...
void pad_and_pack_col(int lda, double *new, double *old)
{
  ...
  if (lda % BLOCK_SIZE != 0)
    newlda = (lda/BLOCK_SIZE)*BLOCK_SIZE + BLOCK_SIZE; // if odd dimensioned matrix, pad
...
void unpad_and_unpack_col(int lda, double *packed, double *unpacked)
{
  ...
  if (lda % BLOCK_SIZE != 0)
    newlda = (lda/BLOCK_SIZE)*BLOCK_SIZE + BLOCK_SIZE;
...
void square_dgemm (int lda, double* A, double* B, double* C)
{
  ...
  for (int j = 0; j < newlda; j += BLOCK_SIZE)
	...
	do_block(newA + k*newlda + i*BLOCK_SIZE, 
          newB + k*newlda + j*BLOCK_SIZE, newC + j*newlda + i*BLOCK_SIZE);
      }
